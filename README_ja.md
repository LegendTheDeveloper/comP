...
「comP: Setup Agents」を実行すると、あなたのマシン用に `command` パスと `COMP_WORKSPACE_ROOT` が入力済みの `.comp/config/claude_desktop_config.json` が生成されます。
`Claude Code` を選択すると、comP はそのままターミナルに貼り付けられるコマンドとオプションの LLM 向けプロンプトをポップアップで表示します。
ポップアップを見逃した場合は `.comp/config/claude_desktop_config.json` を開き、`command` のパスを使用してください。
...
VSCode のチャット（Copilot Chat 等）で直接 `@comp` を使って、長大なファイルを自動圧縮しながら質問できます：

```markdown
@comp #file:src/main.rs を使って、この関数の動作を説明して
```

添付されたファイルは comP のスケルトンモードで自動的に圧縮（コメント削除や関数ボディの `{ ... }` 置き換え）されて LLM に渡るため、トークン消費を最小限に抑えつつ正確な回答を得られます。
...
| **v0.5** | クリップボードへのコード圧縮コピーコマンド（`copyActiveFileCompressed`）、VSCode Chat Participant API による `@comp` チャット参加者機能、Parquet (.parquet) ファイルの自動インデックス化および BM25 全文検索サポート。 | ✅ **リリース済み** |
