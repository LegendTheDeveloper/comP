...
comP は Claude Code・Cursor・Cline・Antigravity などの AI エージェントが、コードベースを効率的に理解・分析できるよう支援します。プロジェクトを自動でインデックスし、セマンティックなコードグラフを構築し、トークン数を算出します—全てローカルで完結します。
...
3. comP が自動的にバックグラウンドで **インデックス処理を開始**
...
| **v0.2** | Word サポート、高度な影響分析 | ⚪ 計画中 |
| **v0.3** | 埋め込みベース検索、複数リポジトリ対応 | ⚪ 計画中 |
| **v0.4** | プロジェクトドキュメント自動生成、Git 差分対応コンテキスト（PR レビュー・影響範囲検知） | ⚪ 計画中 |
