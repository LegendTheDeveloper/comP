...
///
/// WHY multiplicative and small: the old +0.25 additive boost, exempt from the
/// cutoff, put CLAUDE.md and two unrelated HTML templates among the pivots of
/// "calculate performance points" only because they were unsaved work. A
/// dirty file with no evidence now goes to `working_tree_files` instead.
pub const GIT_DIFF_FACTOR: f32 = 1.15;
/// Share of the symbol component that comes from weighted keyword coverage
/// (how much of the query's keyword weight this file matches) versus the
/// single best hit. Coverage is what separates a file that matches three of
...
/// Keywords matching more than this share of the code files are skipped for
/// LIKE and filename search (their top rows are arbitrary); they still
/// participate in TF-IDF and BM25, whose own IDF handles common terms.
///
/// WHY files, not nodes: the old 10 %-of-nodes rule never fired ("steam"
/// matched 1 739 nodes of 47 396, most of them one autogenerated enum file).
/// Measured on Lynium with 1 978 code files: "on" 61 %, "get" 22 %, "mode"
/// 13 % become common; "beatmap" 7.7 %, "steam" 6.3 % stay searchable.
pub const COMMON_KEYWORD_FILE_SHARE: f64 = 0.08;
/// Keywords at or above this rarity weight are "defining" for the task:
/// confidence degrades when they produce no code evidence, and only they can
/// match symbol-less files (shaders, text) by name.
...
/// A symbol that matched a task keyword, kept so the response can point the
/// agent at a line instead of at a whole file.
...
/// Rarity weight of every keyword that was actually searched (noise and
/// corpus-common ones excluded), so a file's coverage can be expressed as
/// "share of the query's total keyword weight it matches".
#[derive(Debug, Clone, Default)]
pub struct QueryWeights {
    pub by_kw: HashMap<String, f32>,
    pub total: f32,
}

impl QueryWeights {
    pub fn new<'a>(pairs: impl IntoIterator<Item = (&'a str, f32)>) -> Self {
        let mut by_kw = HashMap::new();
        let mut total = 0.0f32;
        for (kw, w) in pairs {
            let w = w.max(0.0);
            if by_kw.insert(kw.to_lowercase(), w).is_none() {
                total += w;
            }
        }
        QueryWeights { by_kw, total }
    }
}
...
pub struct Evidence {
    ...
    pub sym_best: f32,
    /// Best match quality per keyword that produced a symbol/filename hit.
    pub kw_quality: HashMap<String, f32>,
    ...
    pub bm25_raw: f64,
    /// The symbols behind the hits (deduplicated by name, best quality kept).
...
impl Evidence {
    pub fn add_symbol_hit(&mut self, keyword: &str, quality: f32, kw_weight: f32) {
        ...
        let entry = self.kw_quality.entry(keyword.to_lowercase()).or_insert(0.0);
        if quality > *entry {
            *entry = quality;
        }
    }

    /// Remember the symbol behind a hit. Bounded per file; a weaker hit
    /// replaces nothing, a stronger one replaces the weakest remembered.
    pub fn record_hit(&mut self, hit: SymbolHit) {
        if let Some(existing) = self.hits.iter_mut().find(|h| h.name == hit.name) {
            if hit.quality > existing.quality {
                *existing = hit;
            }
            return;
        }
        ...
        if let Some(weakest) = self
            .hits
            .iter_mut()
            .min_by(|a, b| a.quality.partial_cmp(&b.quality).unwrap_or(std::cmp::Ordering::Equal))
        {
            if hit.quality > weakest.quality {
                *weakest = hit;
            }
        }
    }
    ...
    /// Share of the query's keyword weight this file covers, each keyword
    /// contributing its best match quality. 0 when nothing was searchable.
    pub fn weighted_coverage(&self, q: &QueryWeights) -> f32 {
        if q.total <= 0.0 {
            return 0.0;
        }
        let covered: f32 = q
            .by_kw
            .iter()
            .map(|(kw, w)| self.kw_quality.get(kw).copied().unwrap_or(0.0) * w)
            .sum();
        (covered / q.total).clamp(0.0, 1.0)
    }

    /// Symbol component: weighted coverage blended with the single best hit.
    ///
    /// WHY not max-plus-bonus: the old bonus counted keywords as rare as
    /// "user" or "game" and let eleven `*Model.cs` files, each matching the
    /// one word "model" at token quality, outrank `GameSettings.cs` for a
    /// five-word task. Coverage makes breadth across the task's own words
    /// the main signal; sym_best keeps a single exact rare hit meaningful.
    pub fn symbol_component(&self, q: &QueryWeights) -> f32 {
        if q.total <= 0.0 {
            return self.sym_best.min(1.0);
        }
        (W_COVERAGE * self.weighted_coverage(q) + W_SYM_BEST * self.sym_best).min(1.0)
    }

    /// Best hits first (quality desc, then line), at most `max`.
    pub fn matched_symbols(&self, max: usize) -> Vec<SymbolHit> {
...
/// Coverage of a group of files taken together (each keyword at its best
/// quality across the group): how much of the task the top pivots explain.
pub fn union_coverage(evs: &[&Evidence], q: &QueryWeights) -> f32 {
    if q.total <= 0.0 {
        return 0.0;
    }
    let covered: f32 = q
        ...
        .map(|(kw, w)| {
            evs.iter()
                .map(|e| e.kw_quality.get(kw).copied().unwrap_or(0.0))
                .fold(0.0f32, f32::max)
                * w
        })
        .sum();
...
/// Second opinion on `assess_confidence`, taken after ranking.
///
/// WHY: raw signals said "high" for 63 of 99 real queries and never "weak",
/// including the query whose pivots were eleven post-processing models. The
/// coverage of the top three pivots over the task's own words, and whether
/// the first pivot is the project's code, are what the agent can actually
/// rely on. `top_coverage` is None when no keyword was searchable (all noise
/// or corpus-common), in which case only the raw signals decide.
pub fn calibrate_confidence(
    base: &'static str,
    base_weak: bool,
    sig: &RawSignals,
    top_coverage: Option<f32>,
    top_is_first_party: bool,
) -> (&'static str, bool) {
    if base_weak {
        return ("low", true);
    }
    let strong_raw = sig.strong_exact
        || sig.max_tfidf_raw >= STRONG_TFIDF_RAW
        || sig.max_bm25_raw >= STRONG_BM25_RAW;
    match top_coverage {
        Some(cov) if cov < LOW_TOP_COVERAGE && !strong_raw => ("low", true),
        Some(cov) if base == "high" && cov >= HIGH_TOP_COVERAGE && top_is_first_party => ("high", false),
        Some(_) => ("medium", false),
        None => (base, false),
    }
}

/// Languages stored without symbols (findable by name only).
pub fn is_symbol_less_language(lang: &str) -> bool {
    matches!(lang, "shader" | "text" | "unknown")
}
...
pub fn combine_score(
    ...
    let score = W_SYMBOL * ev.symbol_component(q) + W_TFIDF * tfidf_norm + W_BM25 * bm25_norm;
    if git_diff {
        score * GIT_DIFF_FACTOR
    } else {
        score
    }
}
...
pub fn match_reasons(ev: &Evidence, git_diff: bool) -> Vec<String> {
    ...
    if ev.sym_best > 0.0 {
        let mut kws: Vec<&str> = ev.kw_quality.keys().map(|s| s.as_str()).collect();
        kws.sort();
        if kws.is_empty() {
...
/// floor or below ratio x top_score. No exemptions: a dirty file with no
/// evidence is reported in `working_tree_files`, not smuggled into the pivots.
...
mod tests {
    ...
    fn weights(pairs: &[(&str, f32)]) -> QueryWeights {
        QueryWeights::new(pairs.iter().map(|(k, w)| (*k, *w)))
    }

    fn hit(name: &str, line: i32, quality: f32) -> SymbolHit {
        SymbolHit { name: name.to_string(), kind: "method".to_string(), line, quality }
    }

    #[test]
    fn test_weighted_coverage_shares_query_weight() {
        let q = weights(&[("settings", 0.5), ("model", 0.7), ("stored", 0.8)]);
        let mut ev = Evidence::default();
        ev.add_symbol_hit("settings", 1.0, 0.5);
        ev.add_symbol_hit("model", 0.85, 0.7);
        // (1.0*0.5 + 0.85*0.7) / 2.0
        assert!((ev.weighted_coverage(&q) - 0.5475).abs() < 1e-4);
        assert_eq!(Evidence::default().weighted_coverage(&q), 0.0);
        assert_eq!(ev.weighted_coverage(&QueryWeights::default()), 0.0);
    }

    #[test]
    fn test_coverage_beats_single_generic_exact_match() {
        // The PostProcessing case: an exact "Settings" plus a token "Model"
        // against a file that matches three of the five task words.
        let q = weights(&[("settings", 0.5), ("model", 0.6), ("stored", 0.8), ("changed", 0.7), ("player", 0.5)]);
        let mut bloom = Evidence::default();
        bloom.add_symbol_hit("settings", 1.0, 0.5);
        bloom.add_symbol_hit("model", 0.85, 0.6);
        let mut game_settings = Evidence::default();
        game_settings.add_symbol_hit("settings", 0.85, 0.5);
        game_settings.add_symbol_hit("stored", 0.85, 0.8);
        game_settings.add_symbol_hit("changed", 0.85, 0.7);
        assert!(game_settings.symbol_component(&q) > bloom.symbol_component(&q));
        // Without searchable keywords the component falls back to the best hit.
        assert_eq!(bloom.symbol_component(&QueryWeights::default()), bloom.sym_best);
    }

    #[test]
    fn test_record_hit_dedups_and_bounds() {
        let mut ev = Evidence::default();
        ev.record_hit(hit("Save", 10, 0.4));
        ev.record_hit(hit("Save", 10, 0.85));
        assert_eq!(ev.hits.len(), 1);
        assert_eq!(ev.hits[0].quality, 0.85);
        for i in 0..MAX_HITS_PER_FILE + 3 {
            ev.record_hit(hit(&format!("Sym{}", i), i as i32, 0.5));
        }
        assert_eq!(ev.hits.len(), MAX_HITS_PER_FILE);
        // A stronger hit still gets in by replacing the weakest.
        ev.record_hit(hit("Exact", 1, 1.0));
        assert!(ev.hits.iter().any(|h| h.name == "Exact"));
        let top = ev.matched_symbols(2);
        assert_eq!(top[0].name, "Exact");
        assert_eq!(top.len(), 2);
    }

    #[test]
    fn test_union_coverage_takes_best_per_keyword() {
        let q = weights(&[("replay", 1.0), ("spectate", 1.0)]);
        let mut a = Evidence::default();
        a.add_symbol_hit("replay", 1.0, 1.0);
        let mut b = Evidence::default();
        b.add_symbol_hit("spectate", 0.85, 1.0);
        assert!((union_coverage(&[&a, &b], &q) - 0.925).abs() < 1e-4);
        assert!((union_coverage(&[&a], &q) - 0.5).abs() < 1e-4);
    }

    #[test]
    fn test_calibrate_confidence_rules() {
        let strong = RawSignals { max_sym_quality: 1.0, strong_exact: true, ..Default::default() };
        let mild = RawSignals { max_sym_quality: 0.85, ..Default::default() };
        // High needs coverage AND a first-party top pivot.
        ...
        assert_eq!(calibrate_confidence("medium", false, &strong, Some(0.1), true), ("medium", false));
        // Base weak stays weak; no searchable keywords leaves the base verdict.
        assert_eq!(calibrate_confidence("low", true, &strong, Some(0.9), true), ("low", true));
        assert_eq!(calibrate_confidence("high", false, &strong, None, true), ("high", false));
    }
    ...
    fn test_evidence_max_not_sum() {
        let q = weights(&[("board", 0.5), ("survey", 0.8)]);
        let mut ev = Evidence::default();
        ...
        assert!(exact.symbol_component(&q) > ev.symbol_component(&q));
    }

    #[test]
    fn test_evidence_distinct_keyword_coverage() {
        let q = weights(&[("survey", 0.8), ("feedback", 0.8)]);
        ...
        assert!(two.symbol_component(&q) > one.symbol_component(&q));
        // Full coverage with exact hits saturates at 1.0
        let mut many = Evidence::default();
        many.add_symbol_hit("survey", 1.0, 1.0);
        many.add_symbol_hit("feedback", 1.0, 1.0);
        assert_eq!(many.symbol_component(&q), 1.0);
    }

    #[test]
    fn test_combine_two_engines_beat_one() {
        let q = weights(&[("survey", 0.8)]);
        ...
        let s_both = combine_score(&both, &q, 0.5, 0.0, false);
        let s_single = combine_score(&single, &q, 0.5, 0.0, false);
        assert!(s_both > s_single);
    }

    #[test]
    fn test_combine_git_diff_is_multiplicative() {
        let q = weights(&[("survey", 0.8)]);
        let mut ev = Evidence::default();
        ev.add_symbol_hit("survey", 0.85, 0.8);
        let plain = combine_score(&ev, &q, 0.0, 0.0, false);
        let boosted = combine_score(&ev, &q, 0.0, 0.0, true);
        assert!((boosted - plain * GIT_DIFF_FACTOR).abs() < 1e-6);
        // No evidence: nothing to boost.
        assert_eq!(combine_score(&Evidence::default(), &q, 0.0, 0.0, true), 0.0);
    }
    ...
    #[test]
    fn test_cutoff_no_longer_exempts_git_diff() {
        let cfg = RelevanceConfig::default();
        let cands = vec![
            cand("top.rs", 0.9, false),
            cand("editing.rs", 0.01, true), // fails both thresholds
        ];
        let (kept, dropped) = apply_cutoff(cands, &cfg);
        assert_eq!(dropped, 1);
        assert!(kept.iter().all(|c| c.path != "editing.rs"));
    }
    ...
    #[test]
    fn test_common_keywords_report_but_do_not_weigh() {
        // Keywords that were not searchable (noise / corpus-common) are absent
        // from QueryWeights, so matching them adds no coverage; the reasons
        // still show them (needed to diagnose brand noise).
        let q = weights(&[("survey", 0.8)]);
        let mut ev = Evidence::default();
        ev.add_symbol_hit("launcher", 0.85, 0.2);
        ev.add_symbol_hit("player", 0.85, 0.3);
        assert_eq!(ev.kw_quality.len(), 2, "reporting keeps all matched keywords");
        assert_eq!(ev.weighted_coverage(&q), 0.0);
    ...
    fn test_filename_hit_marks_reason_and_scores() {
        ...
        assert!(ev.symbol_component(&weights(&[("survey", 0.8)])) > 0.0);
