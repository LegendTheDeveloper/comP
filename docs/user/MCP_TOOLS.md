...
- `confidence` is calibrated after ranking: `high` needs the top three pivots to cover at least 0.4 of the task's keyword weight (`coverage.top_coverage`, the top quarter of answers on the measured workspaces) and a first-party first pivot; coverage under 0.2 with no strong raw signal is `low` with `weak_results`.
...
- `index_ready` (boolean): the daemon serves the MCP handshake before its initial indexing pass ends and builds the TF-IDF index only at the end of that pass, so the first query of a session used to run against a half-built index without anyone knowing. `run_pipeline` now waits up to 45 s for the pass to finish; `false` means it was still running and the answer lacks the TF-IDF channel.
